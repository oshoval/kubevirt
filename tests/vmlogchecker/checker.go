...
var VirtLauncherErrorAllowlist = []AllowlistEntry{
	...
	{
		ID:    12,
		Regex: regexp.MustCompile(`"level":"error","msg":"Getting the domain failed\.",".*"reason":"virError\(Code=.*, Domain=.*, Message='(internal error: client socket is closed|Cannot recv data: Connection reset by peer|End of file while reading data: Input/output error)'\)"`),
		SIGs:  SIGCompute | SIGNetwork | SIGStorage,
	},
	{
		ID:    13,
	...
	{
		ID:    101,
		Regex: regexp.MustCompile(`"level":"error","msg":"Failed to signal deletion for vmi",".*"reason":"virError\(Code=.*, Domain=.*, Message='(Cannot recv data: Connection reset by peer|internal error: client socket is closed|End of file while reading data: Input/output error)'\)"`),
		SIGs:  SIGCompute | SIGNetwork | SIGStorage,
	},
	{
		ID:    102,
