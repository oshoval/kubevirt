...
func ValidateVirtualMachineInstanceSpec(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec, config *virtconfig.ClusterConfig) []metav1.StatusCause {
	...
	causes = append(causes, validateNetworkDevicesWithDRAGateEnabled(field, spec, config)...)
...
func validateNetworkDevicesWithDRAGateEnabled(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec, config *virtconfig.ClusterConfig) []metav1.StatusCause {
	var causes []metav1.StatusCause

	if dra.HasNetworkDRA(spec.Networks) && !config.NetworkDevicesWithDRAGateEnabled() {
		causes = append(causes, metav1.StatusCause{
			Type:    metav1.CauseTypeFieldValueInvalid,
