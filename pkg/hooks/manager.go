	...
	for uint(len(processedSockets)) < numberOfRequestedHookSidecars {
		...
		for _, entry := range entries {
			...
			for _, subEntry := range subEntries {
				...
				err := handleSidecarSocket(filepath.Join(subPath, subEntry.Name()), subEntry.Name(), callbacksPerHookPoint, processedSockets)
				if err != nil {
					return nil, err
				}
			}
...
func handleSidecarSocket(filePath string, subEntryName string, callbacksPerHookPoint map[string][]*callBackClient, processedSockets map[string]bool) error {
	...
	processedSockets[subEntryName] = true
