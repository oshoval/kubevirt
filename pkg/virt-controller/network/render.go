...
func GetNetworkAttachmentDefinitions(networkClient networkclient.Interface, namespace string, multusNetworks []virtv1.Network) (map[string]*networkv1.NetworkAttachmentDefinition, error) {
	...
	for _, network := range multusNetworks {
		...
		nad, err := networkClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(namespace).Get(context.Background(), networkName, metav1.GetOptions{})
		if err != nil {
