...
func WithIPAMClaimRef(networkToIPAMClaimParams map[string]libipam.IPAMClaimParams) network.Option {
	return func(mnap network.MultusNetworkAnnotationPool, namespace string, networks []virtv1.Network) network.MultusNetworkAnnotationPool {
		for _, net := range vmispec.FilterMultusNonDefaultNetworks(networks) {
			...
			i, element := mnap.FindMultusAnnotation(namespace, networkName, networkToIPAMClaimParams[net.Name].InterfaceName)
			if element == nil {
				continue
			}
