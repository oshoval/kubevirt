	...
	if c.needsSync(key) && syncErr == nil {
		...
		if c.clusterConfig.HotplugNetworkInterfacesEnabled() &&
			...
			if hasOrdinalIfaces, err := c.hasOrdinalNetworkInterfaces(vmi); err != nil {
				...
				if updatedVmiSpec, err := network.ApplyDynamicIfaceRequestOnVMI(c.clientset.NetworkClient().K8sCniCncfIoV1(), vmCopy, vmiCopy, hasOrdinalIfaces); err != nil {
					syncErr = &syncErrorImpl{fmt.Errorf("Error encountered when trying to apply dynamic interface request on vmi: %v", err), HotPlugNetworkInterfaceErrorReason}
				} else {
