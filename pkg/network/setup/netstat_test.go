...
var _ = Describe("netstat", func() {
	...
	Context("with volatile cache", func() {
		...
		It("run status and expect two interfaces/networks to be reported (without guest-agent)", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryPodIPv4, primaryPodIPv6}, "", "", netvmispec.InfoSourceDomain, 1),
				newVMIStatusIface(secondaryNetworkName, []string{secondaryPodIPv4, secondaryPodIPv6}, "", "", netvmispec.InfoSourceDomain, 1),
		...
		It("run status and expect interface/network to be reported with the right quque count (without guest-agent)", func() {
			queueCount := uint(8)
			...
			Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryPodIPv4, primaryPodIPv6}, "", "", netvmispec.InfoSourceDomain, queueCount),
		...
		It("run status and expect 2 interfaces to be reported based on guest-agent data", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryGaIPv4, primaryGaIPv6}, primaryMAC, primaryIfaceName, netvmispec.InfoSourceDomainAndGA, 1),
				newVMIStatusIface(secondaryNetworkName, []string{secondaryGaIPv4, secondaryGaIPv6}, secondaryMAC, secondaryIfaceName, netvmispec.InfoSourceDomainAndGA, 1),
		...
		It("run status and expect an interfaces (with masquerade) to be reported based on pod & guest-agent data", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryPodIPv4, primaryPodIPv6}, primaryMAC, "eth0", netvmispec.InfoSourceDomainAndGA, 1),
			}), "the pod IP/s should be reported in the status")
		...
		It("should update existing interface status with MAC from the domain", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryPodIPv4, primaryPodIPv6}, newDomainMAC, "", netvmispec.InfoSourceDomain, 1),
			}), "the pod IP/s should be reported in the status")
		})
	...
	It("should update existing interface status with IP from the guest-agent", func() {
		...
		Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
			newVMIStatusIface(primaryNetworkName, []string{newGaIPv4, newGaIPv6}, origMAC, primaryIfaceName, netvmispec.InfoSourceDomainAndGA, 1),
		}), "the pod IP/s should be reported in the status")
	})
	...
	It("should report SR-IOV interface when guest-agent is inactive and no other interface exists", func() {
		...
		Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
			newVMIStatusIface(networkName, nil, ifaceMAC, "", netvmispec.InfoSourceDomain, 1),
	...
	It("should report SR-IOV interface when guest-agent is inactive and a regular interface exists", func() {
		...
		Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
			newVMIStatusIface(primaryNetworkName, []string{primaryPodIPv4}, "", "", netvmispec.InfoSourceDomain, 1),
			newVMIStatusIface(networkName, nil, "", "", netvmispec.InfoSourceDomain, 1),
		}), "the SR-IOV interface should be reported in the status.")
	})
	...
	It("should report SR-IOV interface with MAC and network name, based on VMI spec and guest-agent data", func() {
		...
		Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
			newVMIStatusIface(networkName, nil, ifaceMAC, guestIfaceName, netvmispec.InfoSourceDomainAndGA, 1),
	...
	When("the desired state (VMI spec) is not in sync with the state in the guest (guest-agent)", func() {
		...
		It("reports masquerade and bridge interfaces with their MAC changed in the guest", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(ConsistOf([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryPodIPv4, primaryPodIPv6}, primaryMAC, "", netvmispec.InfoSourceDomain, 1),
				newVMIStatusIface(secondaryNetworkName, nil, secondaryMAC, "", netvmispec.InfoSourceDomain, 1),
				newVMIStatusIface("", []string{primaryGaIPv4, primaryGaIPv6}, newMAC1, primaryIfaceName, netvmispec.InfoSourceGuestAgent, 0),
				newVMIStatusIface("", []string{secondaryGaIPv4, secondaryGaIPv6}, newMAC2, secondaryIfaceName, netvmispec.InfoSourceGuestAgent, 0),
			}))
		})
		...
		It("reports a new interface that appeared in the guest", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(ConsistOf([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryPodIPv4, primaryPodIPv6}, primaryMAC, primaryIfaceName, netvmispec.InfoSourceDomainAndGA, 1),
				newVMIStatusIface(secondaryNetworkName, []string{secondaryPodIPv4, secondaryPodIPv6}, secondaryMAC, secondaryIfaceName, netvmispec.InfoSourceDomainAndGA, 1),
				newVMIStatusIface("", []string{newGaIPv4, newGaIPv6}, newMAC1, newIfaceName, netvmispec.InfoSourceGuestAgent, 0),
			}))
		})
		...
		It("reports that an interface is not seen in the guest", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(ConsistOf([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryPodIPv4, primaryPodIPv6}, primaryMAC, "", netvmispec.InfoSourceDomain, 1),
				newVMIStatusIface(secondaryNetworkName, []string{secondaryPodIPv4, secondaryPodIPv6}, secondaryMAC, "", netvmispec.InfoSourceDomain, 1),
			}))
		})
	})
	...
	Context("backward compatability", func() {
		...
		It("reports no infoSource when virt-launcher is old and only the domain data exists (but GA is active)", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, nil, primaryMAC, "", netvmispec.InfoSourceDomain, 1),
			}))
		})
		...
		It("reports no infoSource when virt-launcher is old and both the domain & GA data exists", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(primaryNetworkName, []string{primaryGaIPv4}, primaryMAC, primaryIfaceName, netvmispec.InfoSourceDomainAndGA, 1),
			}))
		})
	})
	...
	Context("misc scenario", func() {
		...
		It("has interface in domain spec but not in VMI spec", func() {
			...
			Expect(setup.Vmi.Status.Interfaces).To(Equal([]v1.VirtualMachineInstanceNetworkInterface{
				newVMIStatusIface(networkName, nil, MAC, "", netvmispec.InfoSourceDomain, 1),
			}))
		})
...
func newVMIStatusIface(name string, IPs []string, mac, ifaceName string, infoSource string, queueCount uint) v1.VirtualMachineInstanceNetworkInterface {
	var ip string
	if len(IPs) > 0 {
		ip = IPs[0]
	}
