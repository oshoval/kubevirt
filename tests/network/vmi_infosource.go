...
var _ = SIGDescribe("Infosource", func() {
	...
	Context("VMI with 3 interfaces", func() {
		...
		const (
			...
			secondaryInterface1NewMac = "02:00:a0:a0:a0:a0"
			secondaryInterface2Name   = "bridge2-setns"
			secondaryInterface2NewMac = "02:00:a1:a1:a1:a1"
			dummyInterfaceMac         = "02:00:b0:b0:b0:b0"
		)

		secondaryNetwork1 := libvmi.MultusNetwork(nadName, secondaryInterface1Name)
		secondaryNetwork2 := libvmi.MultusNetwork(nadName, secondaryInterface2Name)
		...
		BeforeEach(func() {
			...
			vmiSpec := libvmi.NewFedora(
				...
				libvmi.WithInterface(*libvmi.InterfaceWithMac(&secondaryLinuxBridgeInterface1, secondaryInterface1NewMac)),
				libvmi.WithInterface(*libvmi.InterfaceWithMac(&secondaryLinuxBridgeInterface2, secondaryInterface2NewMac)),
		...
		It("should have the expected entries in vmi status", func() {
			expectedInterfaces := []kvirtv1.VirtualMachineInstanceNetworkInterface{
				...
				},
				{
					...
					InterfaceName: "eth1",
					MAC:           secondaryInterface1NewMac,
					Name:          secondaryInterface1Name,
				},
				{
					InfoSource: netvmispec.InfoSourceDomain,
					MAC:        secondaryInterface2NewMac,
			...
			orderedInterfaces := orderInterfaceStatus(vmiObj.Status.Interfaces)
			for i := range orderedInterfaces {
				expectedInterfaces[i].IP = orderedInterfaces[i].IP
				expectedInterfaces[i].IPs = orderedInterfaces[i].IPs
			}

			Expect(reflect.DeepEqual(expectedInterfaces, orderedInterfaces)).To(BeTrue(),
				fmt.Sprintf("interfaces doesn't match\n%v\n%v", expectedInterfaces, orderedInterfaces))
		})
	})
})
...
// orderInterfaceStatus orders "guest-agent" only records in predictable order
func orderInterfaceStatus(interfaces []kvirtv1.VirtualMachineInstanceNetworkInterface) []kvirtv1.VirtualMachineInstanceNetworkInterface {
	orderedArray := interfaces
	length := len(orderedArray)
	if orderedArray[length-1].InterfaceName != dummyInterfaceName {
		orderedArray[length-1], orderedArray[length-2] =
			orderedArray[length-2], orderedArray[length-1]
	}
